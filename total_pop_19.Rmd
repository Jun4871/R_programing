...
# 평일만 뺀 데이터
Monday <- Wday_grouping_1 %>% 
  filter(Wday == "Monday")
Tuesday <- Wday_grouping_1 %>% 
   filter(Wday == "Tuesday")
Wednesday <- Wday_grouping_1 %>% 
   filter(Wday == "Wednesday") 
Thursday <- Wday_grouping_1 %>% 
    filter(Wday == "Thursday")
Friday <- Wday_grouping_1 %>% 
  filter(Wday == "Friday") 


Weekday_for_compare <- as.data.frame(rbind(Monday, Tuesday, Wednesday, Thursday, Friday))
...
ggplot(Weekend_for_compare, aes(x= Wday, y= Wday_Pop)) +
  geom_col() +
   scale_y_continuous(labels =  comma)+
    ggtitle( "19년 주말 유동인구 그래프", subtitle = ("기간 : 19.03 ~ 19.12")) +
  labs(x= "요일", y="유동인구 수")  +
   theme(axis.text.x = element_text(size=20)) +
  theme(axis.text.y = element_text(size=20))
```
