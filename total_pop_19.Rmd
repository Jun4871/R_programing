...
Time_grouping <- year_19_pop_data_frame %>% 
  ...
  arrange(desc(Time_Pop)) %>% as.data.frame()
...
ggplot(Month_grouping, aes(x=reorder(Month, -Mon_pop), y=Mon_pop)) + 
  ...
  scale_y_continuous(labels =  comma)
   # coord_flip() 
  # scale_y_continuous(breaks = c(2764211800, 3158641740))
  # coord_cartesian(ylim=c(2764211800, 3158641740))

#ax(Month_grouping$Mon_pop)
  #  theme(axis.title.x=element_text(family="NanumGothic",  
  #                                       face="bold",          
  #                                       hjust=0.5,              
  #                                       size=25,               
  ...
  #                                       color="Black",
  #                                       angle=0)) +
  # scale_y_continuous(limits=c(min(Month_grouping$Mon_pop),
  #                             max(Month_grouping$Mon_pop)),
  #                             labels=scales::comma) +
  # theme(axis.text.x = element_text(size=13, face ="bold")) +
  # theme(axis.text.y = element_text(size=20))
...
# geom_bar(stat="identity") +
  # geom_col(mapping = aes(fill = Month),  width = 0.7,
  #          alpha = 0.9) +
  # geom_bar(stat = "identity") +
 # geom_line(size=5, color="blue",  linetype = 1) +
 # geom_point(size=5,alpha=9, shape=21,color = "#ff6600",
...
# hist(Month_grouping$Mon_pop)

# ggplot(x = Month_grouping$Month y=Month_grouping$Mon_pop,
#      breaks = 10,
#      ylim = c(0,35),
#      labels = TRUE,
...
# 
# 
# # 이 모델을 test셋에다가 했을때 어떤 결과를 나타내냐
